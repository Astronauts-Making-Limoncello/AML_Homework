...
# Left / right indicator
    ...
    if pred:
        lcolor = "#9b59b6" # dark violet
        rcolor = "#2ecc71" # green 
    else:
        lcolor = "#8e8e8e" # grey 1
        rcolor = "#383838" # grey 2
    ...
    for i in np.arange( len(I) ):
        ...
        if not update:
            ...
            if i ==0:
                ...
                plots.append(ax.plot(x, y, z, lw=2,linestyle='-' if pred else '-.' ,c=lcolor if LR[i] else rcolor,label=['Input Frames' if not pred else 'Predicted Frames']))
            else:
                plots.append(ax.plot(x, y, z, lw=2,linestyle='--', c=lcolor if LR[i] else rcolor))
                # plots.append(ax.plot(x, y, z, lw=2,linestyle='--', c=lcolor if LR[i] else rcolor))
                plots.append(ax.plot(x, y, z, lw=2,linestyle='-' if pred else '-.', c=lcolor if LR[i] else rcolor))
    ...
    for action in actions:
      ...
      # joints at same loc
        ...
        for cnt,batch in enumerate(loader): 
            ...
            # print(f"batch.shape: {batch.shape}") # batch, n_frames, n_joints*3
            # print("\n\n")
            ...
            # print(f"batch[:, 0:input_n, dim_used].shape: {batch[:, 0:input_n, dim_used].shape}")
            # print(f"sequences_train.shape: {sequences_train.shape}") # batch, xyz, n_inputs, n_joints 
            # print("\n\n")
            sequences_gt=batch[:, input_n:input_n+output_n, :]
            # print(f"sequences_gt.shape: {sequences_gt.shape}")
            # print("\n\n")
            
            # print(f"modello(sequences_train).shape: {modello(sequences_train).shape}")
            # print(f"modello(sequences_train).permute(0,1,3,2).shape: {modello(sequences_train).permute(0,1,3,2).shape}")
            ...
            # sequences_predict = modello(sequences_train)[:, 0:input_n, dim_used].view(-1,input_n,len(dim_used)//3,3).permute(0,3,1,2).contiguous().view(-1,output_n,len(dim_used))
