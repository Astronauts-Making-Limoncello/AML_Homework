...
def main(use_progress_bars=False, use_rich_print=False):
  ...
  if use_progress_bars: progress_bar.start()
  ...
  def train(data_loader,vald_loader, path_to_save_model=None):
    ...
    train_loss = []
    train_loss_mpjpe = []
    train_loss_special_tokens = []
    val_loss = []
    val_loss_mpjpe = []
    val_loss_special_tokens = []
    train_loss_best = 100000000
    train_loss_mpjpe_best = 100000000
    train_loss_special_tokens_best = 100000000
    val_loss_best   = 100000000
    val_loss_mpjpe_best   = 100000000
    val_loss_special_tokens_best   = 100000000
    ...
    for epoch in range(conf.n_epochs-1):
        ...
        for cnt,batch in list(enumerate(data_loader))[:n_train_batches]:
          ...
          running_loss_mpjpe += loss_mpjpe * batch_dim
          running_loss_special_tokens += loss_special_tokens * batch_dim
            ...
            torch.save(
              {
                ...
                'train_loss_mpjpe': train_loss_mpjpe,
                'train_loss_special_tokens': train_loss_special_tokens,
                'val_loss': val_loss,
                'val_loss_mpjpe': val_loss_mpjpe,
                'val_loss_special_tokens': val_loss_special_tokens
              }, 
              f"{ckpt_dir}/{conf.model_name}_best_train_loss.pt"
            )

          if running_loss_special_tokens/n < train_loss_special_tokens_best:
            train_loss_special_tokens_best = running_loss_special_tokens/n
            ...
            torch.save(
              ...
              f"{ckpt_dir}/{conf.model_name}_best_train_loss_special_tokens.pt"
            )
          
          if running_loss_mpjpe/n < train_loss_mpjpe_best:
            train_loss_mpjpe_best = running_loss_mpjpe/n
            ...
            torch.save(
              ...
              f"{ckpt_dir}/{conf.model_name}_best_train_loss_mpjpe.pt"
            )
        ...
        train_loss_mpjpe.append(running_loss_mpjpe.detach().cpu()/n)
        train_loss_special_tokens.append(running_loss_special_tokens.detach().cpu()/n)
        ...
        with torch.no_grad():
            ...
            if running_loss_mpjpe/n < val_loss_mpjpe_best:
              val_loss_mpjpe_best = running_loss_mpjpe/n
              ...
              torch.save(
                ...
                f"{ckpt_dir}/{conf.model_name}_best_val_loss_mpjpe.pt"
              )
            
            if running_loss_special_tokens/n < val_loss_special_tokens_best:
              val_loss_special_tokens_best = running_loss_special_tokens/n
              ...
              torch.save(
                ...
                f"{ckpt_dir}/{conf.model_name}_best_val_loss_special_tokens.pt"
              )


            val_loss.append(running_loss.detach().cpu()/n)
            val_loss_mpjpe.append(running_loss_mpjpe.detach().cpu()/n)
            val_loss_special_tokens.append(running_loss_special_tokens.detach().cpu()/n)
        ...
        wandb.log({
          ...
          "loss/mpjpe/train": train_loss_mpjpe[-1],
          "loss/special_tokens/train": train_loss_special_tokens[-1],
          "loss/val": val_loss[-1],
          "loss/mpjpe/val": val_loss_mpjpe[-1],
          "loss/special_tokens/val": val_loss_special_tokens[-1]
        })
        ...
        if epoch == 0:
          ...
          print(
            f"epoch: [bold][#B22222]{(epoch + 1):04}[/#B22222][/b] | " +
            f"train loss: [bold][#6495ED]{train_loss[-1]:07.3f}[/#6495ED][/b], best (step): {train_loss_best:07.3f} | " + 
            f"val loss: [b][#008080]{val_loss[-1]:07.3f}[/#008080][/b], best: {val_loss_best:07.3f}" + 
            f"\n" + 
            f"epoch: [bold][#B22222]{(epoch + 1):04}[/#B22222][/b] | " +
            f"train mpjpe: [bold][#6495ED]{train_loss_mpjpe[-1]:07.3f}[/#6495ED][/b], best (step): {train_loss_mpjpe_best:07.3f} | " + 
            f"val mpjpe: [b][#008080]{val_loss_mpjpe[-1]:07.3f}[/#008080][/b], best: {val_loss_mpjpe_best:07.3f}" + 
            f"\n" + 
            f"epoch: [bold][#B22222]{(epoch + 1):04}[/#B22222][/b] | " +
            f"train special_tokens: [bold][#6495ED]{train_loss_special_tokens[-1]:07.3f}[/#6495ED][/b], best (step): {train_loss_special_tokens_best:07.3f} | " + 
            f"val special_tokens: [b][#008080]{val_loss_special_tokens[-1]:07.3f}[/#008080][/b], best: {val_loss_special_tokens_best:07.3f}" + 
            f"\n\n"
          )
    ...
    wandb.log({
      ...
      "best_loss/mpjpe/train": np.array(train_loss_mpjpe).min(),
      "best_loss/special_tokens/train": np.array(train_loss_special_tokens).min(),
      "best_loss_epoch/train": np.array(train_loss).argmin() + epoch_offset,
      "best_loss/val": np.array(val_loss).min(),
      "best_loss/mpjpe/val": np.array(val_loss_mpjpe).min(),
      "best_loss/special_tokens/val": np.array(val_loss_special_tokens).min(),
  ...
  def test(ckpt_path, final_epoch_print):
      ...
      for action in actions:
        ...
        for cnt,batch in list(enumerate(test_loader))[:n_test_batches]:
          ...
          with torch.no_grad():
            ...
            accum_loss_mpjpe+=loss_mpjpe*batch_dim
            accum_loss_special_tokens+=loss_special_tokens*batch_dim


        action_loss_dict[f"loss/test/{action}"] = np.round((running_loss/n).item(),1)
        action_loss_dict[f"loss/mpjpe/test/{action}"] = np.round((running_loss_mpjpe/n).item(),1)
        action_loss_dict[f"loss/special_tokens/test/{action}"] = np.round((running_loss_special_tokens/n).item(),1)

        n_batches+=n

        if use_progress_bars: progress_bar.advance(task_id=actions_task, advance=1)
      ...
      action_loss_dict["loss/mpjpe/test"] = np.round((accum_loss_mpjpe/n_batches).item(),1)
      action_loss_dict["loss/special_tokens/test"] = np.round((accum_loss_special_tokens/n_batches).item(),1)
      
      
      action_loss_dict["best_loss/test"] = np.round((accum_loss/n_batches).item(),1)
      action_loss_dict["best_loss_mpjpe/test"] = np.round((accum_loss_mpjpe/n_batches).item(),1)
      action_loss_dict["best_loss_special_tokens/test"] = np.round((accum_loss_special_tokens/n_batches).item(),1)
...
if __name__ == "__main__":
  main(use_progress_bars=True, use_rich_print=True)