...
class SpatioTemporalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"\[SpatioTemporalAttention.forward] xs.shape: {xs.shape}")

        q, k = torch.chunk(self.to_qkvs(xs).view(N, 2 * self.num_heads, self.qkv_dim, T, V), 2, dim=1)
        # print(f"\[SpatioTemporalAttention.forward] q.shape: {q.shape}")
        # print(f"\[SpatioTemporalAttention.forward] k.shape: {k.shape}")

        attention = self.tan(torch.einsum('nhctu,nhctv->nhuv', [q, k]) / (self.qkv_dim * T)) * self.alphas
        # print(f"\[SpatioTemporalAttention.forward] attention.shape: {k.shape}")
